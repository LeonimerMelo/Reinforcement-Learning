# -*- coding: utf-8 -*-
"""
Created on Mon Mar  3 17:19:33 2025
...
env = gym.make('CartPole-v1', render_mode = 'rgb_array', max_episode_steps = max_episode_steps_)
...
ReplayMemory - a cyclic buffer of bounded size that holds the transitions 
...
Q-network
=========
Our model will be a feed forward neural network that takes in the difference between 
the current and previous screen patches. It has two outputs, representing 
Q(s,left) and Q(s,right) (where s is the input to the network). In effect, 
the network is trying to predict the expected return of taking each action given 
the current input.
'''    
class DQN(nn.Module):
...
we’ll just sample one uniformly. The probability of choosing a random action 
will start at EPS_START and will decay exponentially towards EPS_END. 
...
'''
plot_durations - a helper for plotting the duration of episodes, along with an 
...
# train the agent  
num_episodes = 550
rewards_hist = [] # Lista para armazenar as recompensas
mean_loss = []
cont = 0
solved = False
epsilon_hist = []
...
epsilon_decay_rate = 6/num_episodes # epsilon decay rate
...
for episode in range(num_episodes):
    ...
    for t in count():
        ...
        rewards += reward # Salva a recompensa recebida
    ...
    print('episode:', episode, 'steps:', t+1)
    
    if len(loss_hist) > 0:
        mean_loss.append(np.mean(loss_hist))
        loss_hist = []
    ...
    if rewards >= env.spec.max_episode_steps:  
...
plt.ylabel("loss")
plt.grid(True)
plt.plot(mean_loss, color="black")
plt.show()
