...
env = gym.make("Taxi-v3", render_mode ='rgb_array' if is_training else 'human',
...
if not is_training:
    ...
    for i in range(20):
        ...
        while not done:
            ...
            print(f"step: {step:<5} action: {act[action]:<8} reward: {reward} info: {info["action_mask"]} new state: {new_state}")
            state = new_state
            step += 1
            if terminated or truncated:
                done = True
