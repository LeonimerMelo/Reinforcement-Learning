...
epsilon_decay_rate = 6/episodes # Fator de decaimento do epsilon
...
if is_training:
    ...
    media_movel_ = media_movel(rewards_per_episode, janela=200)
    plt.plot(media_movel_, color="orange")
    ...
    plt.plot(media_movel_, color="green", alpha=0.8)
...
# best policy index for each state
state_argmax = np.argmax(q_table, axis=1)  
# policy
directions = {0: "←", 1: "↓", 2: "→", 3: "↑"}
# replace with arrows
state_argmax_ = []
for i in range(len(q_table)):
    if np.max(q_table[i]) == 0.0:
        state_argmax_.append('X') # frozen lake position
    else:
        state_argmax_.append(directions[state_argmax[i]])
# Goal position
state_argmax_[len(q_table)-1]='G'

# Remodelar a matriz para o formato do grid
state_argmax_mat = state_argmax.reshape(rows, cols)
# Substitiur os valores pelas setas
state_argmax_str = np.array(state_argmax_, dtype=str).reshape(rows, cols)
# Plot the optimal policy
# plt.figure(figsize=(8, 6))
sns.heatmap(state_argmax_mat, annot=state_argmax_str, 
            # cmap=sns.color_palette("Blues", as_cmap=True), 
            linewidths=0.7, fmt="", linecolor="black", 
            xticklabels=[], yticklabels=[], annot_kws={"fontsize": "xx-large"})
plt.title("Learned Q-value: optimal policy\nArrows represent best action")
plt.show()