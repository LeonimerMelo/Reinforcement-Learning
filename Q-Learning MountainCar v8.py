...
The action is a ndarray with shape (1,), representing the directional force 
applied on the car. The action is clipped in the range [-1,1] and multiplied by 
a power of 0.0015.
...
'''
Action Space - Mountain Car
There are 3 discrete deterministic actions:
0: Accelerate to the left
1: Don’t accelerate
2: Accelerate to the right

Action Space - Mountain Car Continuous
The action is clipped in the range [-1,1]
'''
MountainCarMode = 'MountainCar-v0'
# MountainCarMode = 'MountainCarContinuous-v0'

max_episode_steps_ = 1000 # truncate afther this
# if is training no render
env = gym.make(MountainCarMode, render_mode=None if is_training else 'human', 
